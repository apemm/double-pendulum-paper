...
moves. The velocity does evolve at first: $\omega_1$ falls by exactly $h$ per
step for 2048 steps, until at $t=\tfrac12$ it reaches $-\tfrac12$, where $h$ is
half a unit in the last place of $\omega_1$ and round-to-even freezes it. The
increment to the angle is then $h\,|\omega_1|=2^{-13}$ for ever, a quarter of the
$2^{-11}$ needed to change the last of eleven bits in $\theta_1=\pi/2$, and the
pendulum hangs horizontally with a constant velocity that produces no motion.
With the same eleven bits and $h=2^{-8}$ the increment first exceeds $2^{-11}$ at
$t=0.129$ and the pendulum falls. The pattern is general for this release: the
velocity freezes at $2^{p-k}$ and the angle needs $|\omega_1|>2^{k-p}$, so the
pendulum stalled for every $h<2^{-p}$ we tried and moved for every $h>2^{-p}$,
while at $h=2^{-p}$ the increment is itself a tie and the outcome depends on the
last bit of $\pi/2$. A smaller step has made the error total.
...
multiplied by about \costFactor\ and some \extraBits\ more bits are needed. At the
finite-time rate \lamStep\ of the first 160 time units the figures would be
nearer \costFactorFT\ and \extraBitsFT. Either way the horizon grows as the
logarithm of the effort. In everyday units the long-time exponent is
$\lambda=\lamSI$~s$^{-1}$ for one-metre rods, and a tenfold reduction of the step
buys $4\ln10/\lambda\approx\decadeSI$~s (\decadeSIFT~s at the finite-time rate),
which is what we saw in our first crude runs.
