...
Lyapunov exponent. That is what is offered here.
\ifdefined\shortversion
The exercise teaches that the time for which a simulation of a chaotic system is
accurate depends only logarithmically on the error committed per step; that
truncation and roundoff can be isolated once the precision of the arithmetic is a
parameter of the program; that numerical error, being a small perturbation,
measures the Lyapunov exponent as well as a perturbed initial condition does;
that each floating-point format has a best step and a longest accurate time; and
that conservation of energy is not evidence that a chaotic trajectory is correct.
\else
...
\ifdefined\shortversion
Pendulums that start from identical states and are integrated with identical
237-bit arithmetic, differing only in $h$, coincide to within the width of a line
for a long time and then depart one by one, largest step first. An animation in
the supplementary material, in which the error curves are drawn alongside the
pendulums as they evolve, makes the point to a class more quickly than any graph.
\else
...
students. With $p=11$ and $h=2^{-12}$ the pendulum \emph{never moves}, and the way
it fails repays a minute's thought. The velocity does evolve at first: $\omega_1$
falls by exactly $h$ at every step for 2048 steps. At $t=\tfrac12$ it reaches
$-\tfrac12$, where $h$ is half a unit in the last place of $\omega_1$; the sum is
a tie, round-to-even returns $-\tfrac12$, and the velocity is frozen. From then on
the increment to the angle is $h\,|\omega_1|=2^{-13}$, a quarter of the $2^{-11}$
needed to change the last of eleven bits in $\theta_1=\pi/2$, and the computed
pendulum hangs horizontal forever with a constant velocity that produces no
motion. With the same eleven bits and $h=2^{-8}$ the velocity would not freeze
until $|\omega_1|=8$, the increment $h\,|\omega_1|$ first exceeds $2^{-11}$ at
$t=0.129$, and the pendulum falls.
\ifdefined\shortversion\else
The pattern is general for this release. The
velocity freezes at $2^{p-k}$ and the angle needs $|\omega_1|>2^{k-p}$, so the
pendulum stalled for every $h<2^{-p}$ that we tried and moved for every
$h>2^{-p}$; at $h=2^{-p}$ the increment is itself a tie, and the outcome depends
on the last bit of $\pi/2$.
\fi
Here a smaller step has not reduced the error but
made it total, and no analysis based on small perturbations describes what
happens. It is an extreme case of the tendency expressed by the negative sign in
Eq.~(\ref{eq:Tp}).
...
bits. Which $\lambda$ to use depends on the interval in question. Doubling the
horizon attainable in double precision means extrapolating beyond the times
sampled here, for which the long-time exponent \lamBen\ is the appropriate rate:
the number of steps is multiplied by roughly
$e^{\lambda T_{\max}/4}\approx\costFactor$ and some \extraBits\ additional bits
are called for. At the finite-time rate \lamStep\ measured over the first 160 time
units the figures would be nearer \costFactorFT\ and \extraBitsFT. Either way the
horizon grows as the logarithm of the effort spent on it. A method of order $n$ replaces the 4 by $n$, which is
...
In everyday units the result is easy to remember. For rods one meter long the
long-time exponent is $\lambda=\lamSI~\mathrm{s^{-1}}$, so reducing the step
tenfold buys $4\ln10/\lambda\approx\decadeSI$~s of accurate motion, or
\decadeSIFT~s at the finite-time rate, at ten times the cost.
...
\ifdefined\shortversion
The exercise suits an upper-division course in computational physics, mechanics
or numerical analysis. The program is the usual RK4 with the number type changed,
and all but the two finest reference runs complete in minutes on a laptop.
Students with only hardware arithmetic can still do the third experiment, using
double precision at the best step of Table~\ref{tab:best} as the reference for
single precision.
\else
