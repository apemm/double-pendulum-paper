...
def write_numbers(S):
    ...
    m = {
        ...
        "costFactorFT": f"{float(f'{np.exp(F['step']['lambda'] * Tdbl / 4):.1g}'):.0f}",
        "extraBitsFT": f"{F['step']['lambda'] * Tdbl * (4 + S['gamma']['median']) / (4 * LN2):.0f}",
        "decadeSIFT": f"{4 * np.log(10) / (F['step']['lambda'] / sec):.0f}",
        "lamSI": f"{lam / sec:.2f}",
